...
#This PCA is correcting for the fact that we did not collect 25cm soil at each of the days so that data
#may make certain survey days missing
#And this time we are making sure that the 0s in mushroom richness actually show up but that will be corrected for in 
#The all variables code
...
complete_everything<-soil10cm%>%
  left_join(mushroom_richness, by=c("survey","site","plot","plot_number"))%>%
...
ggplot(complete_everything, aes(days_since_first_survey, mushroom_richness, fill=site))+geom_boxplot()
...
ggbiplot(pc,
         obs.scale=1,
         var.scale=1,
         groups=complete_everything$site,
         ellipse=TRUE,
         circle=TRUE,
         ellipse.prob=0.68
) 
